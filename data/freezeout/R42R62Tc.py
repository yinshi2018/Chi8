...
dif22cen=abs(r42mub22cen-0.122195)
...
r42=[max22[154],min22[154]]
r62=[max6222[154],min6222[154]]
r4222up=np.max(r42)
...
for num in range(1,20):
    if num==1:
       max68=np.maximum(r4268[:,num-1],r4268[:,num])
       min68=np.minimum(r4268[:,num-1],r4268[:,num])
    else:
       max68=np.maximum(max68,r4268[:,num])
       min68=np.minimum(min68,r4268[:,num])
...
dif68cen=abs(r42mub68cen-0.302545)
...
r42=[max68[144],min68[144]]
r62=[max6268[144],min6268[144]]
r4268up=np.max(r42)
...
dif106cen=abs(r42mub106cen-0.432666)
...
r42=[max106[137],min106[137]]
r62=[max62106[137],min62106[137]]
r42106up=np.max(r42)
...
dif148cen=abs(r42mub148cen-0.302545)
...
r42=[max148[129],min148[129]]
r62=[max62148[129],min62148[129]]
r42148up=np.max(r42)
...
dif196cen=abs(r42mub196cen-0.612299)
...
r42=[max196[129],min196[129]]
r62=[max62196[129],min62196[129]]
r42196up=np.max(r42)
...
for num in range(1,100):
    if num==1:
       max303=np.maximum(r42303[:,num-1],r42303[:,num])
       min303=np.minimum(r42303[:,num-1],r42303[:,num])
    else:
       max303=np.maximum(max303,r42303[:,num])
       min303=np.minimum(min303,r42303[:,num])
...
dif303cen=abs(r42mub303cen-0.680635)
...
r42=[max303[104],min303[104]]
r62=[max62303[104],min62303[104]]
r42303up=np.max(r42)
...
for num in range(1,100):
    if num==1:
       max406=np.maximum(r42406[:,num-1],r42406[:,num])
       min406=np.minimum(r42406[:,num-1],r42406[:,num])
    else:
       max406=np.maximum(max406,r42406[:,num])
       min406=np.minimum(min406,r42406[:,num])
...
dif406cen=abs(r42mub406cen-0.60824)
...
r42=[max406[89],min406[89]]
r62=[max62406[89],min62406[89]]
r42406up=np.max(r42)
...
line_mub,=ax7.plot(xsame,r42mub303cen,'b',linewidth=1,alpha=0.5)
ax7.plot(data11,y62)
...
ax16.errorbar(energy,r42cen,yerr=r42err,color='blue',marker='o',linestyle='',linewidth=2,markersize=5,fillstyle='none',alpha=1)#,label=r'$$')
#ax16.plot(xsame,data)
#ax16.legend(( ),(r'This work $T=155$ MeV'),loc=0,fontsize='x-small',frameon=True,shadow=True,handlelength=3.,borderpad=0.5,borderaxespad=1,numpoints=1)
ax16.set_xscale('log')
...
fig.subplots_adjust(top=0.95, bottom=0.1, left=0.16, right=0.95, hspace=0.35,
                    wspace=0.35)


fig.savefig("R42R62tc.pdf")
