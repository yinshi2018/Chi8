...
\begin{table}[t]
  \begin{center}
    ...
    \hline & & & & & & & & & \\[-2ex]
    $T_{_{CF}}$ [MeV] &151 & 154 & 156 & 156 & 156 & 155 & 151 & 150 & 144\\[1ex]
    \hline & & & & & & & & & \\[-2ex]
    $T_{c}$ [MeV] & 164 & 164 & 164 & 163 & 162 & 161 & 158 & 155 & 146\\[1ex]
    \hline & & & & & & & & & \\[-2ex]
    $T_{_{CF}}$ [MeV] & 151 & 155 & 159 & 155 & 162 & 161 & 158 & 147 & 128\\[1ex]
    \hline
    \end{tabular}
    \caption{Chemical freeze-out parameters ${\mu_B}_{_{CF}}$ and $T_{_{CF}}$, and the pseudocritical temperature $T_c$ for different center-of-mass energies per nucleon pair $\sqrt{s_{NN}}$, and see text for details.}
...
\caption{$R^{B}_{32}$ (upper left), $R^{B}_{42}$ (lower left), $R^{B}_{62}$ (upper right), and $R^{B}_{82}$ (lower right) as functions of the collision energy, calculated in LEFT within the fRG approach on the freeze-out line and the pseudocritical line, respectively. Experimental data from the STAR collaboration are also shown for comparison, where $R^{p}_{32}$ (upper left) and $R^{p}_{42}$ (lower left) are the skewness and kurtosis of the net-proton distributions measured in Au+Au central (0-5\%) collisions \cite{Adam:2020unf}, and $R^{p}_{62}$ (upper right) is the preliminary result on the six-order cumulant of the net-proton distribution at $\sqrt{s_{NN}}$=200 GeV and 54.4 GeV with centrality 0-40\% \cite{Nonaka:2020crv,Pandav:2020uzx}.}\label{fig:Rm2-sqrtS}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
In this work we will not take into account the non-critical and dynamical effects discussed above when the theoretical results are confronted with experiments, but rather assume that the measured cumulants of the net-proton multiplicity distribution at a given collision energy, with other collision parameters e.g., the centrality and rapidity range fixed, is in one-by-one correspondence to the calculated fluctuations in \Eq{eq:suscept} with one value of $T$ or $\mu_B$. It is reasonable to attribute the values of $T$ and $\mu_B$ to be the ones when the chemical freeze-out occurs, viz. $T_{_{CF}}$ and ${\mu_B}_{_{CF}}$. Such an approach for the comparison is usually employed in fluctuation studies of equilibrium QCD matter within functional methods or lattice simulations \cite{Fu:2015amv,Isserstedt:2019pgx,Bazavov:2020bjn}. Note, however, that because of the reasons we have outlined above, results of comparison between the theory and experiment within this simplified approach should be taken with a grain of salt. As we will see below, the theoretically predicted dependence of baryon number fluctuations on the collision energy is less sensitive to ${\mu_B}_{_{CF}}$ in contradistinction to $T_{_{CF}}$. Hence we adopt a phenomenological relation between the freeze-out chemical potential and the center-of-mass energy per nucleon pair $\sqrt{s_{NN}}$, which reads
%
\begin{align}
   {\mu_B}_{_{CF}}&=\frac{a}{1+0.288\sqrt{s_{NN}}},\label{}
\end{align}
%
with $a=1307.5$ MeV, which is obtained from the analysis of hadron yields in the hadron resonance gas model \cite{Andronic:2017pug}. As for the freeze-out temperature $T_{_{CF}}$, we follow the method in \cite{Fu:2015amv} and employ the experimentally measured skewness of the net-proton multiplicity distribution to determine $T_{_{CF}}$ for every collision energy, i.e., 
%
\begin{align}
   R^B_{32}(T_{_{CF}})&=R^p_{32}(T_{_{CF}}),\label{eq:TCFR32}
\end{align}
%
where $R^{B}_{32}=\chi^{B}_{3}/\chi^{B}_{2}$ on the l.h.s. is calculated from the theory and that on the right is the skewness of the net-proton distribution measured in the experiment. See the left-top panel in \Fig{fig:Rm2-sqrtS}. Notice that cumulants of net-baryon and net-proton number distributions are assumed implicitly to be equal to each other in \Eq{eq:TCFR32}. Finally, the resulting values of $T_{_{CF}}$ as well as ${\mu_B}_{_{CF}}$ for different collision energies are given in \Tab{tab:freeze-out-para}. Furthermore, in \Tab{tab:freeze-out-para} we also show the pseudocritical temperature $T_c$ extracted from the infection point of the chiral order parameter, i.e., the expectation value of the sigma field $\langle \sigma \rangle$ as a function of $T$ with $\mu_B$ fixed, which in fact give the pseudocritical line in comparison to the freeze-out line. In what follows we will calculate the baryon number fluctuations both on the freeze-out line and the pseudocritical line.

In \Fig{fig:Rm2-sqrtS} we show the dependence of baryon number fluctuations $R^{B}_{32}$, $R^{B}_{42}$, $R^{B}_{62}$, and $R^{B}_{82}$ on the center-of-mass collision energy, which is calculated in the LEFT within the fRG approach on the freeze-out line and pseudocritical line as presented in \Tab{tab:freeze-out-para}. The theoretical calculations are also confronted with experimental measurement of cumulants of the net-proton distributions in the beam energy scan experiments from the STAR collaboration. The skewness ($R^{p}_{32}$) and kurtosis ($R^{p}_{42}$) of the net-proton distributions are measured in Au+Au collisions with centrality 0-5\%, transverse momentum range $0.4< p_T\,(\mathrm{GeV}/c)\,<2.0$, and rapidity $|y|<0.5$, cf. \cite{Adam:2020unf} for more details. The preliminary result for the six-order cumulant of the net-proton distribution are obtained at two values of the collision energy, i.e., $\sqrt{s_{NN}}$=200 GeV and 54.4 GeV with centrality 0-40\% \cite{Nonaka:2020crv,Pandav:2020uzx}. 
...
\end{align}
%
where we have introduced a dimensionless glue potential $\bar V_\mathrm{glue}$, and the dependence of the 
...
The infrared cut-off dependence of the glue potential, i.e., the first term on the r.h.s. of \Eq{eq:Vtotal}, is neglected in this work, and it is parameterized as follows
%
\begin{align}
  V_\text{glue}(L,\bar{L})
